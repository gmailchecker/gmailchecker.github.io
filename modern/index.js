...
document.querySelector(".download-result").style.animation="slideup 0.75s"; document.querySelector(".hidedown").style.display="flex";document.querySelector(".solid2").style.display="none";document.querySelector(".showdown").style.display="none";}
	...
	$( ".btn-executor" ).append( $( "<button onClick='clear_all()' id='clear-editor' class='btn-primary'>CLEAR ALL</button>" ) );;
	$( "#clear-editor" ).after( $( "<button onClick='clear_all_run()' id='check-btn' class='check_btn'>CHECK NOW</button>" ) );
    ...
    for (let i = 0; i < smallParts.length; i++) {
        ...
        while (true) {
            ...
            if (result === false) {			
				...
				abp.notify.info(" server has been changed to SERVER 2");
				  model = 2;		
				} else if (model == 3) {
				$("#turbo-btn").hide();			
				$("#active_server1").hide();
				...
				abp.notify.info(" server has been changed to SERVER 1");
				  model = 1;			
				}
